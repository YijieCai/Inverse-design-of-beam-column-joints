...
model = torch.load(r'F:/Python_venv/Deep_Learning/pythonProject/MNIST-CYJ/05 DDPM/diffusionmodel.pt')
model=model.to(device)
model.eval()
diffusiondataset = DiffusionDataset(
    "F:\\Python_venv\\Deep_Learning\\pythonProject\\MNIST-CYJ\\14 Exceed Target\\Dataset_exceed.h5")

for jjj in range(0,1):
    number_section=4
    ...
    work_dir=fr'F:\Python_venv\Deep_Learning\pythonProject\MNIST-CYJ\14 Exceed Target\01Img'

    fig, (ax) = plt.subplots(2, 2)
    for i in range(number_section):
        np.savetxt(f'{work_dir}/0/Target_{i+jjj*100}.csv', xx_list[i], delimiter=',', fmt="%d")
    ...
    ax[1, 0].axis('off')
    ax[1, 1].axis('off')
    plt.savefig(f'{work_dir}/0/Target_section_{jjj*100}.png', bbox_inches='tight', transparent=True)
    plt.close(fig)
    ...
    for i, t in enumerate(noise_scheduler.timesteps):
        ...
        if i == 0 or i==999:
            ...
            for iii in range(number_section):
                ...
                np.savetxt(f'{work_dir}/0/DDPM_Section_{iii+jjj*100}_{i}.csv', xxxx_list[iii], delimiter=',', fmt="%d")
            ...
            ax[1, 0].axis('off')
            ax[1, 1].axis('off')
            plt.savefig(f'{work_dir}/0/DDPM_Section_{jjj*100}_{i}.png', bbox_inches='tight', transparent=True)
            plt.close(fig)
            ...
            for iii in range(number_section):
                ...
                np.savetxt(f'./01Img/0/x_{iii+jjj*100}_{i}.csv' , column_x_list[iii], delimiter=',', fmt="%d")
                np.savetxt(f'./01Img/0/x_{iii+jjj*100}_{i}_.csv', beam_x_list[iii]  , delimiter=',', fmt="%d")
# ***_第iii个截面 第i步骤
