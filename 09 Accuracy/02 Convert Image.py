...
for number in tqdm(range(500)):
    for i in range(1,5):
        img=np.loadtxt(f'./01Img/{number}/x{i}_.csv',delimiter=',')
        img=img.ravel()
        ...
        np.savetxt(f'./01Img/{number}/{i}_.txt',OpenSees_Import)

for number in tqdm(range(500)):
    for i in range(1,5):
        img=np.loadtxt(f'./01Img/{number}/x{i}.csv',delimiter=',')
        img=img.ravel()
        OpenSees_Import=np.empty([0,4])
        for index,Element in enumerate(Element_Abaqus_Load):
            Temp_Import=np.zeros([1,4])
            ...
            Temp_Import[0,3]=int(img[index])
            OpenSees_Import = np.append(OpenSees_Import, Temp_Import, axis=0)
        np.savetxt(f'./01Img/{number}/{i}.txt',OpenSees_Import)
