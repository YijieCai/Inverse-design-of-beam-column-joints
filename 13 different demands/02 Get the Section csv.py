...
for target in range(4):
    if not os.path.exists(f'./02Section/{target}'):
        os.mkdir(f'./02Section/{target}')
    for number in tqdm(range(4)):
        for i in step_list:
            img=np.loadtxt(f'./01Img/{target}/x_{number+target*100}_{i}_.csv',delimiter=',')
            img=img.ravel()
            ...
            np.savetxt(f'./02Section/{target}/x_{number+target*100}_{i}_.txt',OpenSees_Import)
for target in range(4):
    for number in tqdm(range(4)):
        for i in step_list:
            img=np.loadtxt(f'./01Img/{target}/x_{number+target*100}_{i}.csv',delimiter=',')
            img=img.ravel()
            OpenSees_Import=np.empty([0,4])
            for index,Element in enumerate(Element_Abaqus_Load):
                Temp_Import=np.zeros([1,4])
                ...
                Temp_Import[0,3]=int(img[index])
                OpenSees_Import = np.append(OpenSees_Import, Temp_Import, axis=0)
            np.savetxt(f'./02Section/{target}/x_{number+target*100}_{i}.txt',OpenSees_Import)