...
def make_data(data,label,data_dir):
    savepath=os.path.join('..', os.path.join('checkpoint', data_dir, 'Dataset_Test_unweak.h5'))
...
for index in tqdm(range(10000)):
    if index not in Exclusion:
        if index not in Exclusion2:
            features_Column = np.loadtxt(f'{work_dir}\\Train\\Imgs\\{index}\\Column.csv',delimiter=',')
            features_Beam = np.loadtxt(f'{work_dir}\\Train\\Imgs\\{index}\\Beam.csv',delimiter=',')
            labels=np.loadtxt(f'F:\\Python_venv\\Deep_Learning\\pythonProject\\MNIST-CYJ\\04 Mehanical Property\\Test\\{index}.txt')

            input_features.append([features_Column,features_Beam])
