...
for number in tqdm(range(50)):
    for i in range(1,5):
        img=np.loadtxt(f'./{number}/x{i}_.csv',delimiter=',')
        img=img.ravel()
        ...
        np.savetxt(f'./{number}/{i}_.txt',OpenSees_Import)

for number in tqdm(range(50)):
    for i in range(1,5):
        img=np.loadtxt(f'./{number}/x{i}.csv',delimiter=',')
        img=img.ravel()
        OpenSees_Import=np.empty([0,4])
        for index,Element in enumerate(Element_Abaqus_Load):
            Temp_Import=np.zeros([1,4])
            ...
            Temp_Import[0,3]=int(img[index])
            OpenSees_Import = np.append(OpenSees_Import, Temp_Import, axis=0)
        np.savetxt(f'./{number}/{i}.txt',OpenSees_Import)