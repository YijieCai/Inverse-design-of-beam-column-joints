...
class DiffusionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        feature=torch.Tensor(feature).type(torch.float32).squeeze(0)
        mask=torch.Tensor(label).type(torch.float32)
        return (feature,mask)
    def plot_image(self,index):
        fig,(ax)=plt.subplots(nrows=1,ncols=1)
        ax.imshow(np.array(self.feature[index][0]),cmap='bone',alpha=0.5)
        # plt.show()
        # plt.close()
...
if __name__=='__main__':
    diffusiondataset = DiffusionDataset(r"F:\Python_venv\Deep_Learning\pythonProject\MNIST-CYJ\03 Dataset\Train\Dataset_Train.h5")
