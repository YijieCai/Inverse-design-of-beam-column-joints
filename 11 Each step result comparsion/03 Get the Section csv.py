...
for number in tqdm(range(100)):
    for i in range(1,5):
        img=np.loadtxt(f'./01Img/x_{number*10}_{i}_.csv',delimiter=',')
        img=img.ravel()
        ...
        np.savetxt(f'./03Section_Beam_Column/x_{number*10}_{i}_.txt',OpenSees_Import)

for number in tqdm(range(100)):
    for i in range(1,5):
        img=np.loadtxt(f'./01Img/x_{number*10}_{i}.csv',delimiter=',')
        img=img.ravel()
        OpenSees_Import=np.empty([0,4])
        for index,Element in enumerate(Element_Abaqus_Load):
            Temp_Import=np.zeros([1,4])
            ...
            Temp_Import[0,3]=int(img[index])
            OpenSees_Import = np.append(OpenSees_Import, Temp_Import, axis=0)
        np.savetxt(f'./03Section_Beam_Column/x_{number*10}_{i}.txt',OpenSees_Import)