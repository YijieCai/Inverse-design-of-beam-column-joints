...
def single_worker(fanwei,fanwei2):
    ...
    for number in tqdm(fanwei):
        outdir = rf'F:\Python_venv\Deep_Learning\pythonProject\MNIST-CYJ\12 Model accuracy/03Result'
        step_list = [0, 999]
        for iii in step_list:
            ...
            OpenSees_Import = np.loadtxt(rf'F:\Python_venv\Deep_Learning\pythonProject\MNIST-CYJ\12 Model accuracy/02Section/x_{number}_{iii}.txt')
            ...
            OpenSees_Import = np.loadtxt(rf'F:\Python_venv\Deep_Learning\pythonProject\MNIST-CYJ\12 Model accuracy/02Section/x_{number}_{iii}_.txt')
            ...
            ops.recorder('Node', '-file', f"{outdir}\\Disp_{number}_{iii}.txt", "-time", '-node', 6, '-dof', 2, 'disp')
            ops.recorder('Node', '-file', f"{outdir}\\Reaction_{number}_{iii}.txt", "-time", '-node', 6, '-dof', 2, 'reaction')
